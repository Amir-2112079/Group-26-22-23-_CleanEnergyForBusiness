...
function BusinessChat() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4000/businesschat/getAllMessages/${username}`)
      .then((response) => {
        if (response.data) {
          const newChats = {};
  ...
  function onInputSubmit() {
    if (submitEnabled) {
      ...
      if (
        ...
        console.log(msg);
        axios
          .post("http://localhost:4000/businesschat/sendMessage", msg)
          .then((response) => {
            if (response.data === "Message sent") {
              setUserChats((chatData) => ({
                ...
                [selectedChat]: [
                  ...
                  { chatFrom: msg.chatFrom, message: msg.message },
                ],
              }));
              setUpdate(true);
              setMessageInput("");
        ...
        <div className="chatbox-background">
          {selectedChat
            ? currentChats.map((msg) => (
                <div
                  className={`message ${
                    msg.chatFrom === selectedChat ? "received" : "sent"
                  }`}
                >
