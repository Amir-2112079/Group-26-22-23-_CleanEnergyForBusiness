...
function Radar(){
    ...
    function checkErrors(){
        ...
        }else if(markerInfo.lat > 90){
            err.push("- Business Location: Latitude must be between -90 and +90")
        }else if(markerInfo.lat < -90){
            err.push("- Business Location: Latitude must be between -90 and +90")
        }
        if (!markerInfo.lng) {
        ...
        }else if(markerInfo.lng > 180){
            err.push("- Business Location: Longitude must be between -180 and +180")
        }else if(markerInfo.lng < -180){
            err.push("- Business Location: Longitude must be between -180 and +180")
        }
        return err;
